...
const onAuthStateChanged = (func: (user: TaroreadUser | null) => void): Promise<PluginListenerHandle> => {
    // If user is falsy, then user shall be null.
    return TaroreadNative.addListener("onAuthStateChanged", (user: TaroreadUser) => func(!!user ? user : null))
        // Everytime a listener is added, push an update to all listeners.
        // Could be architected differently if this becomes a hassle.
        .then((handle) => {refreshUser(); return handle;});
}
